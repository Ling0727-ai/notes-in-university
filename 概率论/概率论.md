...
- **定义**：若$\forall\theta\in\Theta,\varepsilon>0$有$\lim\limits_{n\to\infty}P\{|\hat{\theta}-\theta|<\varepsilon\}=1$，则称$\hat{\theta}$是相合估计量
...
### 2. 正态总体均值 $\mu$ 的单侧置信区间（$\sigma^2$ 未知）
